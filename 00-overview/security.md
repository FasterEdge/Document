...
| ProxyArea | SSRF、开放代理、查询串密钥泄露 | 设置 `--key` 和 `--allow-hosts`；启用 TLS；限制网络入口 |
...
现有 DontCrack、ProxyArea 和 SimpleWebShell 接口使用 query 参数传递密码或密钥。查询串会进入浏览器历史、访问日志和代理日志。部署时：

- 强制 HTTPS 或可信内网。
- 关闭或脱敏 URL 日志。
- 不在截图、文档和监控标签中展示真实密钥。
- 长期建议演进为 Authorization header，同时保留 query 参数兼容窗口。
